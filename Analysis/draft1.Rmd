...
# there are two missing values(should we remove them since it is only two NA?)
table(is.na(rain.ts))
```
...
plot(min_t_ts, main = "Portland, OR. Minimum Monthly Temp (1940-2019)",
     xlab = "Year",
     ylab = "Temperature (F)")

# there are two missing values(replace the missing value by avg(monthly))
...
# pull out data and clean up 
snow_month <- snow$AVG.or.Total
snow_month <- gsub("M", NA, snow_month)
snow_month <- gsub("T", 0, snow_month)
snow_month <- as.numeric(snow_month)

# this is a time series we could work with
snow_ts <- ts(snow_month, start = c(1940,10), deltat = 1/12)

plot(snow_month, main = "Portland, OR. Monthly Snow (1940-2019)",
     xlab = "Year",
     ylab = "Thickness (Inch)",
     type='l')

# there are one missing values
table(is.na(snow_month))
```
